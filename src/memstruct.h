...
inline void counter_print(counter_t *c)
{
    printf(" %-5d | %-12d | %-7d | %-14s | %-15d | %-15d | %-12d\n", c->i + 1,
           c->startTime, c->duration, c->fifo_name, c->currClients,
           c->servedClients, c->medTime);
}
...
inline void memstruct_print(memstruct_t *m)
{
    ...
    printf("%-6s | %-22s | %-14s | %-33s | %-12s\n", "Balcao",
           "        Abertura", "     Nome", "            Num clientes",
           "Tempo medio");
    printf("%-6s | %-12s | %-7s | %-14s | %-15s | %-15s | %-12s\n", " #",
           "    Tempo", "Duracao", "     FIFO", "em atendimento",
           "  ja atendidos", "atendimento");
